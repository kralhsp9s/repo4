...
function createBot() {
    ...
    const config = {
        ...
        username: randomNick(),
        checkTimeoutInterval: 90000, // Zaman aşımı toleransını 90 saniyeye çıkardık (Hugging Face için)
    ...
    // --- DÜZELTİLEN CHAT LOG VE OTOMATİK GİRİŞ KISMI ---
    bot.on('messagestr', (message, position) => {
        if (position === 'game_info') return; // Ekranın ortasındaki action-bar yazılarını loglama

        // 1. Web paneline ham mesajı gönder (Artık tüm chat akışı web panelinde görünür)
        io.emit('chat-log', { username: '💬 Sunucu Akışı', message: message });

        // 2. 30 Saniye Sonra Atılma Sorunu İçin Otomatik Giriş/Kayıt Sistemi
...
// Web Sunucusunu Başlat (Hugging Face veya Yerel Portu Dinler)
const PORT = process.env.PORT || 7860;
server.listen(PORT, () => {
    console.log(`[🚀] Web paneli aktif port: ${PORT}`);
});
app.use(session({
    secret: process.env.SESSION_SECRET || 'bot-panel-gizli-sifre', 
    resave: false,
