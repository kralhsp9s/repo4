...
function createBot() {
    ...
    const config = {
        ...
        port: parseInt(process.env.MC_PORT) || 25565, // Web portuyla çakışmaması için MC_PORT yapıldı
        username: randomNick(),
...
// Web Sunucusunu Başlat (Render için PORT otomatik okunur)
...
function createBot() {
    ...
    // Bot Oyuna Girdiğinde
    bot.once('spawn', () => {
        console.log("[🟢] Giriş başarılı! Dünyaya indim.");
        botStartTime = Date.now();
        
        // Anti-AFK hareketlerini başlat
        startActions();

        // Web Paneli için Haritayı Başlat (Iframe için)
        try {
            mineflayerViewer(bot, { port: parseInt(process.env.VIEWER_PORT) || 3001, firstPerson: false });
        } catch (vErr) {
            console.log("Harita başlatılamadı (Port kullanımda olabilir):", vErr.message);
        }
    });

    // Sohbet Logu Yakalama
    bot.on('chat', (username, message) => {
        if (username === bot.username) return;
        io.emit('chat-log', { username, message }); // Web arayüzüne gönder
    });
...
function handleDisconnect() {
    ...
    reconnectTimeout = setTimeout(() => {
        reconnectTimeout = null;
        createBot();
    }, 30000); // Zaman aşımını önlemek için güvenli 30 saniye bekleme
}

// Zaman aşımı (Timed out) sorunlarını tamamen bitirmek için 30 dakikada bir otomatik reset
setInterval(() => {
    console.log("[🔄] 30 dakika doldu. Zaman aşımını önlemek için bot yeniden başlatılıyor...");
    if (bot) bot.end();
}, 30 * 60 * 1000);

// İlk bot açılışını tetikle
createBot();


// --- API ROUTE'LARI (WEB PANELİ) ---

// Herkese Açık Sunucu Bilgisi
app.get('/api/server-info', async (req, res) => {
    try {
        const host = process.env.ip || "izmirr.aternos.me";
...
app.post('/api/login', (req, res) => {
    ...
    const adminPass = process.env.sifre || "admin123";
...
io.on('connection', (socket) => {
    socket.on('admin-action', (data) => {
        ...
        // Chate Mesaj Gönderme
        if (data.action === 'send-message') {
            bot.chat(data.message);
        } 
        
        // Konum Bilgisi İsteme
        else if (data.action === 'get-location') {
            if (bot.entity) {
                const pos = bot.entity.position;
                socket.emit('location-result', `X: ${pos.x.toFixed(2)}, Y: ${pos.y.toFixed(2)}, Z: ${pos.z.toFixed(2)}`);
            } else {
                socket.emit('location-result', "Konum alınamadı (Bot yükleniyor)");
            }
        } 
